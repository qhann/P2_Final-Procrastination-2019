...
export default function artworkSketch(s) {
  ...
  s.setup = () => {
    ...
    function displayShipData() {
      ...
      s.text(
        "time: " + parseFloat(timeElapsed).toFixed(1) + "s",
        x + 2,
        y + 55
      );
    }
    ...
    function hasGroundContact() {
      ...
      var yOfPointL =
        percentOfSectionXL * curDotL[1] +
        (1 - percentOfSectionXL) * prevDotL[1];
      var yOfPointR =
        percentOfSectionXR * curDotR[1] +
        (1 - percentOfSectionXR) * prevDotR[1];
